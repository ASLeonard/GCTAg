...
void GRM::merge_grm_streaming(string mgrm_file, string out_file, int memory_budget_gb){
    std::ifstream mgrm(mgrm_file.c_str());
    if(!mgrm){
        LOGGER.e(0, "can't open " + mgrm_file + " to read.");
    }
    ...
    gcta_grm_io::merge_grms_streaming(files, out_file, memory_budget_gb);
}
...
int GRM::registerOption(map<string, vector<string>>& options_in) {
    ...
    if(options_in.find(op_merge_grm_streaming) != options_in.end()){
        ...
        if(options_in[op_merge_grm_streaming].size() == 1){
            options_d["merge_grm_streaming"] = std::stod(options_in[op_merge_grm_streaming][0]);
        }
        else {
            options_d["merge_grm_streaming"] = 0.0;
        }
...
void GRM::processMain() {
    ...
    for(auto &process_function : processFunctions){
        ...
        if(process_function == "merge_grm_streaming"){
            GRM grm;
            const int stream_budget = static_cast<int>(options_d["merge_grm_streaming"]);
            grm.merge_grm_streaming(options["mgrm"], options["out"], stream_budget);
        }
    }
